...
const META_BOXES_FILTER = 'fieldify_meta_boxes';

add_action( 'after_setup_theme', __NAMESPACE__ . '\\register_custom_post_meta' );
/**
 * Registers custom post meta.
 *
 * @return void
 */
function register_custom_post_meta(): void {
	...
	foreach ( $meta_boxes as $meta_box ) {
		...
		foreach ( $fields as $field ) {
			$id     = $field['id'] ?? '';
			$schema = get_item_schema( $field );
			$type   = $schema['type'];

			$args = [
				'type'         => $type,
				'description'  => $field['label'] ?? to_title_case( $id ),
...
add_filter( 'is_protected_meta', __NAMESPACE__ . '\\hide_from_custom_fields', 10, 2 );
/**
 ...
 * @return bool
 */
function hide_from_custom_fields( bool $protected, string $meta_key ): bool {
...
add_action( 'add_meta_boxes', __NAMESPACE__ . '\\add_custom_meta_boxes', 10, 2 );
/**
 * Register meta boxes.
 *
 ...
 * @return void
 */
function add_custom_meta_boxes( string $current_post_type, $object ): void {
	...
	foreach ( $meta_boxes as $meta_box ) {
		...
		$ids[]      = $id;
		$title      = $meta_box['title'] ?? to_title_case( $id );
		$post_types = $meta_box['post_types'] ?? [ 'post' ];
 ...
 * @return array
 */
function get_meta_boxes(): array {
	return apply_filters( META_BOXES_FILTER, [] );
}
 ...
 * @return array
 */
function get_custom_fields( ?string $post_type = null ): array {
	$meta_boxes = get_meta_boxes();
	$fields     = [];
 ...
 * @return array
 */
function get_item_schema( array $field ): array {
	$type_map = [
		'text'     => [
			'type' => 'string',
		],
	...
	if ( $sub_type === 'object' ) {
		...
		foreach ( $sub_fields as $sub_field ) {
			$schema['items']['properties'][ $sub_field['id'] ?? '' ] = get_item_schema( $sub_field );
		}
	}
